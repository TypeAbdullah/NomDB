...
def from_url(url: str):
    if url.endswith(".nom") or url.endswith(".db") or url.endswith(".nomdb") or url.endswith(".dump") or Path(url).exists():
    ...
    if url.startswith("nomdb://") or url.startswith("redis://"):
        try:
            parsed = urlparse(url)
            if parsed.path.endswith(".nom") or parsed.path.endswith(".db") or parsed.path.endswith(".nomdb"):
